...
static const struct regval_list ov5647_2592x1944_10bpp[] = {
	{0x0100, 0x00},
	{0x0103, 0x01},
	{0x3034, 0x1a},
	{0x3035, 0x21},
...
static const struct regval_list ov5647_1080p30_10bpp[] = {
	{0x0100, 0x00},
	{0x0103, 0x01},
	{0x3034, 0x1a},
	{0x3035, 0x21},
...
static const struct regval_list ov5647_2x2binned_10bpp[] = {
	{0x0100, 0x00},
	{0x0103, 0x01},
	{0x3034, 0x1a},
	{0x3035, 0x21},
...
static const struct regval_list ov5647_640x480_10bpp[] = {
	{0x0100, 0x00},
	{0x0103, 0x01},
	{0x3035, 0x11},
	{0x3036, 0x46},
				  ...
				  struct rkmodule_inf *inf)
{
	...
	strscpy(inf->base.lens, sensor->lens_name,
		sizeof(inf->base.lens));
}

static int ov5647_get_channel_info(struct ov5647 *sensor,
				   struct rkmodule_channel_info *ch_info)
{
	if (ch_info->index != PAD0)
		return -EINVAL;
...
static long ov5647_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_CHANNEL_INFO:
		return ov5647_get_channel_info(sensor, arg);
	default:
		return -ENOIOCTLCMD;
	}
}
				  ...
				  unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case RKMODULE_GET_MODULE_INFO:
		size = sizeof(struct rkmodule_inf);
		break;
	case RKMODULE_GET_HDR_CFG:
		size = sizeof(struct rkmodule_hdr_cfg);
		break;
	case RKMODULE_GET_CHANNEL_INFO:
		size = sizeof(struct rkmodule_channel_info);
		break;
	default:
		return -ENOIOCTLCMD;
	}
	...
	if (cmd == RKMODULE_GET_CHANNEL_INFO &&
	    copy_from_user(karg, up, size)) {
		ret = -EFAULT;
		goto out;
	}

	ret = ov5647_ioctl(sd, cmd, karg);
	if (!ret && copy_to_user(up, karg, size))
		ret = -EFAULT;
...
static int ov5647_link_freq_index(u64 link_freq)
{
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(ov5647_link_freq_menu); i++)
		if (ov5647_link_freq_menu[i] == link_freq)
			return i;
			      ...
			      struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
		*v4l2_subdev_get_try_format(sd, cfg, format->pad) = mode->format;
		...
		sensor->mode = mode;
		link_freq_index = ov5647_link_freq_index(mode->link_freq);
		if (WARN_ON(link_freq_index < 0)) {
			mutex_unlock(&sensor->lock);
			return link_freq_index;
		}
		__v4l2_ctrl_s_ctrl(sensor->link_freq, link_freq_index);
...
static int ov5647_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	/* Read-only controls are adjusted when the mode changes. */
...
static int ov5647_init_controls(struct ov5647 *sensor)
{
	...
	link_freq_index = ov5647_link_freq_index(sensor->mode->link_freq);
...
static int ov5647_probe(struct i2c_client *client)
{
	...
	sensor->module_facing = "unknown";
	sensor->module_name = "unknown";
	sensor->lens_name = "unknown";
	...
	sensor->vdd = devm_regulator_get_optional(dev, "vdd");
	...
	snprintf(sd->name, sizeof(sd->name), "m%02u_%s_ov5647 %s",
		 sensor->module_index, facing, dev_name(dev));
